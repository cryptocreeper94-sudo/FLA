          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} style={{ maxWidth: '800px' }}>
            ...
            <div className="flex gap-4 items-center" style={{ flexWrap: 'wrap' }}>
              ...
              <Link to="/engineering" className="flex items-center gap-2 text-cyan" style={{ fontWeight: 600, fontSize: '0.95rem' }}>
                Engineering Brief <ChevronRight size={18} />
              </Link>
            </div>
          </motion.div>
      ...
      <section style={{ background: 'rgba(255,255,255,0.01)', padding: '6rem 0', borderTop: '1px solid var(--border-light)', borderBottom: '1px solid var(--border-light)' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            ...
            <p className="text-muted" style={{ maxWidth: '600px', margin: '0 auto' }}>At 15,000 miles per year, a 10% fuel efficiency improvement translates to <strong style={{ color: 'var(--text-main)' }}>measurable operational savings at fleet scale.</strong></p>
          </div>
      ...
      <section className="container">
        ...
        {(() => {
          const ecosystemItems = [
            { img: '/assets/images/ecosystem/lume-auto.png', name: 'Lume-Auto', desc: 'Petroleum vehicle governance — 42-signal diagnostic engine with fuel coaching, predictive maintenance, and key management.', status: 'Live', statusColor: 'var(--accent-emerald)' },
            { img: '/assets/images/ecosystem/hydrocore.png', name: 'HydroCore', desc: 'Industrial water-flow governance — deterministic monitoring, leak detection, and consumption optimization.', status: 'In specification', statusColor: 'var(--accent-cyan)' },
            { img: '/assets/images/ecosystem/meridian.png', name: 'Meridian', desc: 'Wireless energy infrastructure — overhead canopy charging with deterministic energy routing and cryptographic billing.', status: 'Active · Canopy Charging', statusColor: 'var(--accent-emerald)' },
            { img: '/assets/images/ecosystem/axiom.png', name: 'AXIOM', desc: 'Deterministic cognitive architecture — 120K+ topics, offline-capable, zero-hallucination knowledge engine.', status: 'Integration layer', statusColor: 'var(--text-muted)' },
          ];
          const ecoId = 'ecosystem-carousel';
          const scrollEco = (dir: number) => {
            const el = document.getElementById(ecoId);
            if (el) {
              const card = el.querySelector('.eco-slide') as HTMLElement;
          ...
          return (
            <div style={{ position: 'relative' }}>
              <button onClick={() => scrollEco(-1)} aria-label="Previous" style={{
              ...
              <button onClick={() => scrollEco(1)} aria-label="Next" style={{
              ...
              <div id={ecoId} style={{
                ...
                <style>{`#${ecoId}::-webkit-scrollbar { display: none; }`}</style>
                {ecosystemItems.map((item, i) => (
                  <motion.div key={i} className="eco-slide" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}
                    style={{
                      minWidth: '280px', maxWidth: '320px', flex: '0 0 auto',
                      scrollSnapAlign: 'center', borderRadius: '16px', overflow: 'hidden',
                      border: '1px solid rgba(255,255,255,0.08)', background: 'rgba(10,10,12,0.9)',
                      transition: 'border-color 0.3s, transform 0.3s',
                    }}
                    onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(6,182,212,0.3)'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; }}
                  ...
                  >
                    ...
                    <div style={{ padding: '20px' }}>
                      ...
                      <p className="text-muted" style={{ fontSize: '0.85rem', lineHeight: 1.6, marginBottom: '0.75rem' }}>{item.desc}</p>
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '8px' }}>
                {ecosystemItems.map((_, i) => (
                  <button key={i} aria-label={`Go to slide ${i + 1}`} onClick={() => {
                    ...
                    background: 'var(--accent-cyan)', opacity: 0.4, transition: 'opacity 0.2s',
                  }}
