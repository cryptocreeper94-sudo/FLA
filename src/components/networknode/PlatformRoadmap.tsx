...
const PHASES = [
  ...
  },
  {
    ...
    glow: 'rgba(56,189,248,0.06)',
    apps: [
      { icon: <Truck size={20} />, name: 'Fleet OS', desc: 'Enterprise-wide fleet and asset management — maintenance scheduling, service history tracking, asset utilization analytics, and fleet health monitoring across all FLA subsidiaries. Extends the LumeScan diagnostic pipeline beyond auction inventory to operational vehicles, transport equipment, and facility assets.' },
      { icon: <Fingerprint size={20} />, name: 'Identity OS', desc: 'Enterprise identity, permissions, and role governance. Bridges internal authentication (Okta, Azure AD) with FLA-VL\'s external verification system, providing a single identity framework across all FLA-OS applications with ledger-anchored access control.' },
      { icon: <Search size={20} />, name: 'FLA-PL Explorer', desc: 'A full-featured ledger analytics application — block inspection, validator health, operational audit trails, and cross-system event tracing. Gives compliance teams and operations leadership direct visibility into every recorded event across the enterprise.' },
      { icon: <Zap size={20} />, name: 'Meridian Energy', desc: 'Wireless energy routing and management for lot infrastructure. Overhead anchor nodes deliver metered power to EV charging stations, lighting, and operational equipment. Every watt is tracked, timestamped, and ledger-verified — the same deterministic trust applied to energy that the FLA-PL applies to vehicle records.' },
    ],
  },
];
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          <div style={{ textAlign: 'center', marginBottom: '3rem' }}>
            ...
            <h2 style={{ fontSize: '2.2rem', marginBottom: '0.75rem', lineHeight: 1.1 }}>
              Native FLA-OS Applications
            </h2>
            <p className="text-muted" style={{ maxWidth: '620px', margin: '0 auto', fontSize: '1rem', lineHeight: 1.7 }}>
              Every app runs natively inside the FLA-OS operating environment, writes to the FLA Private Ledger, and inherits Lume-V governance. The platform grows — the architecture doesn't change.
            </p>
          </div>
        </motion.div>
