...
const SCRIPT = `The greatest challenge in automotive enterprise modernization is not software. It's legacy infrastructure. Billions of dollars are locked in auction systems, dealer networks, and fleet operations that cannot be ripped and replaced. Enter the Cox Enterprise Platform. A deterministic operations layer that structurally embeds legacy systems in a modern, zero-trust governance fabric, without replacing them.

But governing operations is only half the battle. Cox Automotive manages millions of vehicles across Manheim, Autotrader, Kelley Blue Book, and dozens of interconnected divisions. Each division needs its own secure operational record, but the enterprise needs unified trust.

The Cox Enterprise Platform solves this with a Fractal Ledger Architecture. At the center sits CORE, the Certified Operational Root Engine, the universal enterprise backbone. Connected to CORE, every division operates its own isolated dual-ledger. A private Cox Automotive Ledger for internal operations. A public Verified Enterprise Trust layer for external verification. Private but verified.
...
A unified, verifiable trust fabric across the entire Cox Automotive portfolio. No rip and replace. Welcome to the Cox Enterprise Platform.`;
...
console.log('🎙️ Generating Cox Enterprise narration via ElevenLabs (Sarah)...');
